...
- 成片预览、下载、合成任务状态与失败说明。
- 固定演示样本：短文本完整闭环、不同模型同镜头对比、失败恢复案例。**三条已于 2026-09-20 落成**（见下）。
- V1 演示收口：`PATCH /api/projects/{id}` 项目设置落库、固定 `v1demo_main` 演示项目、全链路 Playwright 验收。见 `docs/v1-demo-project.md`。

**P6 演示样本已实现（2026-09-20）：**

- `tools/prepare_p6_demo_samples.py` 幂等准备三个样本，只碰 `p6demo_*`：
  - `p6demo_story` 短文本完整闭环——4 个真实付费镜头（2026-08-28 ark `doubao-seedance-2-0-260128`，各 5.09s / 1280×720）+ 本地 FFmpeg 续合的真实成片（1280×720 / 约 20.35s / AAC）；
  - `p6demo_compare` 同镜头多模型对比——同一提示词、同一首帧交给 ark / dashscope / minimax 三家，**实际输出规格互不相同**（1280×720 5.09s / 1110×828 2.02s / 1024×768 4.46s）；
  - `p6demo_recovery` 失败恢复案例——真实的 MiniMax `HTTP 400 invalid task_id (2013)` 失败记录，错误原文逐字保留。
- 准备与演示**全程零外发**：改编走 mock 规划器，镜头视频复制既有真实产物，成片由本地 FFmpeg 合成，不写 `live_*_call_count`。
- 无费用验收 `tools/test_p6_demo_samples.py`（39 项断言，已接入 `run_no_cost_regression.py`）。源资产来自工作库、不在隔离目录内复制，缺失时整项 SKIP 并说明原因。
- 录屏讲稿：`docs/v1-demo-script.md`。

**演示成片规格：** 1280×720、24fps、yuv420p、libx264，与 P6-C/D/E 一致。
