...
串行执行 **51 项**静态检查、Node 单测、服务契约测试与浏览器测试。**每一项用例都拿到自己的数据目录**——驱动器先把历史夹具种成一个模板，再为每条用例拷贝一份，因此用例之间不会观察到彼此的残留状态；三个授权开关统一置 0。结果写入 `output/playwright/stageC/no_cost_regression_report.json`。可加 `--group static|node|python|browser` 或 `--only <关键字>` 只跑其中一部分。
