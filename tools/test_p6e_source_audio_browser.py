"""P6-E 浏览器原声配置恢复闭环。无 FFmpeg 时 SKIP，且不写 p6e-*.png。"""
...
def _cleanup() -> None:
    for project_id in CREATED:
        if not str(project_id).startswith("p6e_"):
            continue
...
def _seed_project(title: str) -> str:
    ...
    with connect() as conn:
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (project_id, title, "P6-E 浏览器原声配置", "cinematic clean realism", "16:9", 5, "auto",
...
def _seed_ready_project(project_id: str) -> None:
    ...
    make_color_clip_with_sine(folder / "b.mp4", color="blue", size="1280x720", duration=1.5, frequency=880)
    ...
    make_sine_wav(folder / "bg.wav", duration=0.8, frequency=220)
...
def main() -> None:
    ...
    if not ffmpeg_available():
        print("SKIP: 浏览器原声配置恢复与成片预览")
        print("SKIP: 未生成 output/playwright/p6e-*.png（禁止用夹具伪装真实成片）")
        print(INSTALL_HINT)
        return
    if not _health_ok():
        raise SystemExit(
